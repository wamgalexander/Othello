...
global en_flag, DEFAULT_PORT, DEFAULT_FILE_NAME
en_flag = True
...
class ComportPanel(wx.Panel):
	def __init__(self, parent, ID):
		...
		self.com_port_textctrl.SetValue(DEFAULT_PORT)
		#self.com_port_textctrl.SetEditable(False)
		...
		self.file_textctrl.SetValue(DEFAULT_FILE_NAME)
		self.file_textctrl.SetEditable(False)
...
class GraphFrame(wx.Frame):
	...
	def connect(self, event):
		...
		port = self.comport_control.com_port_textctrl.GetValue()
...
class SerialThread (threading.Thread):
	...
	def butter_bandpass_filter(self, data, lowcut, highcut, fs, order):
		nyq = 0.5 * fs
		print(type(fs), type(nyq), type(lowcut), type(highcut))
		low = lowcut / nyq
		high = highcut / nyq
		...
		while en_flag:
			...
			if self.data_lock.acquire():
				...
				for i in range(3):
					...
					#list_data = (c*np.array(self.butter_bandpass_filter(packet.psgData[0], 5, 30, 200, 5))).tolist()
